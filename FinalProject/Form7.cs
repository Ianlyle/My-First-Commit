...
namespace FinalProject
{
    public partial class Form7 : Form
    {
        ...
        {
            InitializeComponent();
            conn = new OleDbConnection(@"Provider=Microsoft.ACE.OLEDB.12.0;Data Source=""D:\2ND SEM\APPSDEV\FinalProject\Hotel2.accdb""");
            conn.Open();
        }
        private void loadData()
        {
            OleDbDataAdapter adapter = new OleDbDataAdapter("SELECT * FROM Info2 ORDER BY Name ASC", conn);
            DataTable Info2 = new DataTable();
                adapter.Fill(Info2);
            DataGrid1.DataSource = Info2;
        ...
        {
            OleDbCommand cmd = new OleDbCommand("INSERT INTO Info2 (Name, [Room_Type], [Number_of_Occupancy], [Check_in], [Check_out]) VALUES (@Name, @Room_Type, @Number_of_Occupancy, @Check_in, @Check_out)", conn);
        ...
        private void BtnDelete_Click_1(object sender, EventArgs e)
        {
            ...
            {
                OleDbCommand cmd = new OleDbCommand("DELETE FROM Info2 WHERE Name = @Name", conn);
                cmd.Parameters.AddWithValue("@Name", TxtName.Text);
                cmd.ExecuteNonQuery();
                loadData();
